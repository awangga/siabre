...
def trainModel(matfile,arch):
    ...
    for x in range(1,101):#pembentukan model dari pixel 1-100
            ...
            path=modelfolderpath(matfile)+str(arch)+folderseparator()+str(x)#melakukan set path model
            createmodel(train_data,labelperpx,path,arch)#membuat dan menyimpan model

def TrainSinglePixelModel(matfile,arch,train_data,label,x):
    labelperpx=getlabel(label,x)#mendapatkan label per pixel
    path=modelfolderpath(matfile)+str(arch)+folderseparator()+str(x)#melakukan set path model
    createmodel(train_data,labelperpx,path,arch)#membuat dan menyimpan model
...
def testModel(matfile,arch):
    mat = scipy.io.loadmat(matfile)
    print('load dataset in numpy array')
    testdt,testlb=loadtestandlabel(mat)
    pixel=1
    path=modelfolderpath(matfile)+str(arch)+folderseparator()+str(pixel)
    print('Model path'+path)
    piksel=generatePixel(path,testdt)
    for x in range(2,101):
        path=modelfolderpath(matfile)+str(arch)+folderseparator()+str(x)
...
def getfoldernamefrompath(fullpath):
    return fullpath.split(folderseparator())[-2]
    
def getsubfolderfrompath(fullpath):
    mf=fullpath.split(folderseparator())[-3]
    subf=fullpath.split(folderseparator())[-2]
    return './'+mf+'/'+subf
...
def plotHasil(label,pred,predm,mse,msem,matfile,n,arch):
    fname1=getfigpath(matfile,'resultpict'+folderseparator()+arch,n)
    createfolder(getsubfolderfrompath(fname1))
    rows=['Stimulus','MLP','Miyawaki']
    ...
    fname2=getfigpath(matfile,'resultmse'+folderseparator()+arch,n)
    createfolder(getsubfolderfrompath(fname2))
    fige, axe = plt.subplots(figsize=(15, 5))
    #mseresult,ssimresult,psnrresult,corrresult
    print('mse mlp : ')
    print(mse)
    print('mse miyawaki :')
    print(msem)
    ...
    for i in mse:
        ...
        psnr.append(i[2])
        corr.append(i[3])
    axe.plot(idx, msen, color = 'blue', label = 'mse mlp')
    axe.plot(idx, ssim, color = 'green', label = 'ssim mlp')
    axe.plot(idx, psnr, color = 'red', label = 'psnr mlp')
    axe.plot(idx, corr, color = 'cyan', label = 'corr mlp')
    ...
    for j in msem:
        ...
        psnrm.append(j[2])
        corrm.append(j[3])
    axe.plot(idx, msenm, color = 'magenta', label = 'mse miyawaki')
    axe.plot(idx, ssimm, color = 'yellow', label = 'ssim miyawaki')
    axe.plot(idx, psnrm, color = 'black', label = 'psnr miyawaki')
    axe.plot(idx, corrm, color = 'pink', label = 'corr miyawaki')
    ...
    import PIL
    fnamegab=getfigpath(matfile,'results'+folderseparator()+arch,n)
...
def modelfolderpath(matfile):
    mpath='.'+folderseparator()+matfile.split('_')[2]+'_'+matfile.split('_')[-2]+folderseparator()
    return mpath

def figfile(matfile,n):
    figfolderpath='.'+folderseparator()+matfile.split('_')[2]+'_'+matfile.split('_')[-2]+'_fig'+folderseparator()+str(n)+'.png'
    return figfolderpath

def figrecfile(matfile,n):
    figfolderpath='.'+folderseparator()+matfile.split('_')[2]+'_'+matfile.split('_')[-2]+'_figrec'+folderseparator()+str(n)+'.png'
    return figfolderpath

def getfigpath(matfile,suffix,n):
    import pathlib
    scriptDirectory = pathlib.Path().absolute()
    figfolderpath=str(scriptDirectory)+folderseparator()+matfile.split('_')[2]+'_'+matfile.split('_')[-2]+'_'+suffix+folderseparator()+str(n)+'.png'
...
def msefilename(matfile,arch):
    import pathlib
    scriptDirectory = pathlib.Path().absolute()
    figfolderpath=str(scriptDirectory)+folderseparator()+matfile.split('_')[2]+'_'+matfile.split('_')[-2]+'_mse'+folderseparator()+str(arch)+'.csv'
    return figfolderpath
