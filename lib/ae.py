...
def trainCNNDenoise12(input_train, output_train,input_test, output_test):
    # In[]: Encoder
    denoising_encoder = Sequential([
    Reshape([12, 12, 1], input_shape=[12, 12]),
    ...
    Conv2DTranspose(32, kernel_size=3, strides=3, padding="SAME", activation="selu",
                                  input_shape=[1, 1, 64]),
    ...
    denoising_ae.compile(loss="binary_crossentropy", optimizer=SGD(learning_rate=0.5))
